...
<div class="container-fluid py-4">
    <div class="cp-card">
        <div class="cp-title">Edit Ide Konten</div>
        <div class="cp-subtitle">Perbarui rincian ide konten ini</div>
        ...
        <form action="<?= base_url('content-ideas/update/'.$idea['id']) ?>" method="POST">
            <div class="cp-row full">
                <div class="cp-field">
                    <label>Judul Ide Konten <span class="text-danger">*</span></label>
                    <input type="text" name="title" class="cp-inp" value="<?= esc(old('title', $idea['title'])) ?>" required>
                </div>
            </div>
            ...
            <div class="cp-row full">
                <div class="cp-field">
                    ...
                    <textarea name="description" class="cp-inp"><?= esc(old('description', $idea['description'])) ?></textarea>
                </div>
            </div>
                ...
                <div class="cp-field">
                    ...
                    <select name="content_pillar_id" class="cp-inp">
                        ...
                        <?php foreach($contentPillars as $pillar): ?>
                            <option value="<?= $pillar['id'] ?>" <?= old('content_pillar_id', $idea['content_pillar_id']) == $pillar['id'] ? 'selected' : '' ?>><?= esc($pillar['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="cp-field">
                    ...
                    <select name="content_type_id" class="cp-inp">
                        ...
                        <?php foreach($contentTypes as $type): ?>
                            <option value="<?= $type['id'] ?>" <?= old('content_type_id', $idea['content_type_id']) == $type['id'] ? 'selected' : '' ?>><?= esc($type['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
                ...
                <div class="cp-field">
                    ...
                    <select name="content_format_id" class="cp-inp">
                        ...
                        <?php foreach($contentFormats as $format): ?>
                            <option value="<?= $format['id'] ?>" <?= old('content_format_id', $idea['content_format_id']) == $format['id'] ? 'selected' : '' ?>><?= esc($format['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="cp-field">
                    ...
                    <select name="priority" class="cp-inp" required>
                        <option value="low" <?= old('priority', $idea['priority']) == 'low' ? 'selected' : '' ?>>Low</option>
                        <option value="medium" <?= old('priority', $idea['priority']) == 'medium' ? 'selected' : '' ?>>Medium</option>
                        <option value="high" <?= old('priority', $idea['priority']) == 'high' ? 'selected' : '' ?>>High</option>
                        <option value="urgent" <?= old('priority', $idea['priority']) == 'urgent' ? 'selected' : '' ?>>Urgent</option>
                    </select>
                </div>
            </div>
                ...
                <div class="cp-field">
                    ...
                    <select name="assigned_to" class="cp-inp">
                        ...
                        <?php foreach($users as $user): ?>
                            <option value="<?= $user['id'] ?>" <?= old('assigned_to', $idea['assigned_to']) == $user['id'] ? 'selected' : '' ?>><?= esc($user['full_name'] ?: $user['username']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="cp-field">
                    <label>Status</label>
                    <select name="status" class="cp-inp">
                        <option value="new" <?= old('status', $idea['status']) == 'new' ? 'selected' : '' ?>>New</option>
                        <option value="idea" <?= old('status', $idea['status']) == 'idea' ? 'selected' : '' ?>>Idea</option>
                        <option value="planned" <?= old('status', $idea['status']) == 'planned' ? 'selected' : '' ?>>Planned</option>
                        <option value="converted" <?= old('status', $idea['status']) == 'converted' ? 'selected' : '' ?>>Converted</option>
            ...
            <div class="form-footer">
                ...
                <button type="submit" class="cpb cpb-pri"><i class="bi bi-check-lg"></i> Perbarui Ide</button>
            </div>
        </form>
    </div>
</div>
